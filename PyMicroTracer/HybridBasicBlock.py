...
class HybridBasicBlock(SuperBasicBlock):
    ...
    def extract_ipc_based_on_bbl(self, bbl_size_scheduler=-1, infinite_scheduler=False):
        ...
        if bbl_size_scheduler > 0:
            ...
            window_size = -1
            if infinite_scheduler is False:
                window_size = self._instruction_scheduler_window_size
                print("Backed-end instruction scheduler windows size:{}".format(window_size))
                self.log_handler.info("Backed-end instruction scheduler windows size:{}".format(window_size))
    ...
    def extract_ipc_based_on_rob(self, window_size=-1, data_source=None, save_output=False):
        ...
        avg_ipc = _cal_ipc(instruction_scheduler_window_size=window_size, levels=level_local, total_inst=total)
...
def _cal_ipc(instruction_scheduler_window_size, levels, total_inst):

    from math import ceil
    cycles = 0

    for level in levels:
        cycles = cycles + max(ceil(len(level)/instruction_scheduler_window_size), 1)

    return total_inst / cycles